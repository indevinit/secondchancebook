...
module.exports= function(app) {
    ...
    const regValidation=[check('email')
        ...
        function createUser(req, res) {
            ...
            if (!errors.isEmpty()) {
                ...
                user.save()
                .then((user)=>{
                    return res.json({ok:true, message:"You are registered. Please login."})
                })
                .catch((error)=>{
    ...
    app.get("/",(req,res)=> res.json("this is server"));
    ...
    app.get('/api/current_user', function (req, res) {
      ...
      .not().isEmpty().withMessage('Book title is required')
      .isLength({ min: 2 }).withMessage('Book title should be at least 2 letters'),
      check('description')
      .not().isEmpty().withMessage('Description is required')
      .isLength({ min: 2 }).withMessage('Description should be at least 20 letters'),
      check('location')
      .not().isEmpty().withMessage('Your location is required'),
      check('contact')
      .not().isEmpty().withMessage('Your contact is required'),
      check('price')
